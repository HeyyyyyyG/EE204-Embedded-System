...
void S800_UART_Init(void)
{
  ...
  UARTIntEnable(UART0_BASE, UART_INT_RX | UART_INT_RT);	//Enable UART0 RX,TX interrupt
	IntEnable(INT_UART0);
	UARTFIFOLevelSet(UART0_BASE,UART_FIFO_TX4_8, UART_FIFO_RX7_8);     // FIFO¥•∑¢…Ó∂ //  ∑¢ÀÕ2/8(4B)Ω” ‹FIFO?6/8??(12B)
	UARTStringPut("\r\nHello, world!\r\n");
}
...
void UART0_Handler(void)
{
  ...
  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
	...
	*(mingling+i)='\0';
	...
	if(flag==2)
	if(strcasecmp("AT+STUDENTCCCCCODE",mingling)==0)
		UARTStringPutNonBlocking("CODE517021910882");
	if(strcasecmp("AT+CLASS",mingling)==0)
	{
		flag=0;i=0;
		UARTStringPutNonBlocking("CLASSF1703305");
	}
