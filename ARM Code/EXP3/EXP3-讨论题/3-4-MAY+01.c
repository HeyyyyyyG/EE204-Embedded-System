...
//enum number{01,02,03,04,05,06,07,08,09,10,11};
...
{
	int32_t uart0_int_status,shiwei,gewei,monnumber;
	char sign[1];
	char mon[3];
	 ...
	 while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
		...
		*(cmd+i) = toupper(UARTCharGetNonBlocking(UART0_BASE));
		i++;
	...
	*sign=cmd[3];
	...
	if(strcmp(sign,"+")==0)
	{
		monnumber=monnumber+shiwei*10+gewei;
	}
	else if(strcmp(sign,"-")==0)
		monnumber=monnumber-shiwei*10-gewei;
	if(monnumber<0) monnumber=monnumber+12;
	if(monnumber>11) monnumber=monnumber-12;
	 UARTStringPutNonBlocking(NoToMonth(monnumber));
