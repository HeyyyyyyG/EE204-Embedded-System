...
uint8_t a1,a2,b1,b2,c1,c2,d1,d2,flag=0,count;
char cmd[11];
uint8_t key,i=0;
uint8_t flaglight=false;
int main(void)
{
...
void S800_UART_Init(void)
{
  ...
  UARTConfigSetExpClk(UART0_BASE, ui32SysClock,115200,(UART_CONFIG_WLEN_8 | UART_CONFIG_STOP_ONE |UART_CONFIG_PAR_NONE));
	...
	UARTFIFOLevelSet(UART0_BASE,UART_FIFO_TX4_8, UART_FIFO_RX7_8);     // FIFO´¥·¢Éî¶ //  ·¢ËÍ2/8(4B)½ÓÊÜFIFO?6/8??(12B)
...
{
	int32_t uart0_int_status,shi,fen,j;
	char sign[1];
	char time[6];
	char time0[1];
	char* pointer;
	 ...
	 while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
		...
		*(cmd+i) = UARTCharGetNonBlocking(UART0_BASE);
		i++;
	...
	*time0=(char)('0'+a1);
	...
	//UARTCharPutNonBlocking(UART0_BASE,time[0]);
	UARTCharPut(UART0_BASE,(int32_t)time[0]);
	UARTCharPut(UART0_BASE,(int32_t)time[1]);
	UARTCharPut(UART0_BASE,(int32_t)time[2]);
	UARTCharPut(UART0_BASE,(int32_t)time[3]);
	UARTCharPut(UART0_BASE,(int32_t)time[4]);
   i=0;
...
void S800_GPIO_Init(void)
{
  ...
  GPIOPinTypeGPIOOutput(GPIO_PORTN_BASE, GPIO_PIN_0);			//Set PN0 as Output pin
	GPIOPinTypeGPIOOutput(GPIO_PORTN_BASE, GPIO_PIN_1);			//Set PN0 as Output pin
...
void SysTick_Handler(void)
{
	...
	else
	{
		...
		flaglight = !flaglight;
		if(flaglight) GPIOPinWrite(GPIO_PORTN_BASE,GPIO_PIN_1,0);
