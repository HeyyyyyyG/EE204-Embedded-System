...
char command1[]="AT+CLASS";
char command2[]="AT+STUDENTCODE";
char output1[]="CLASS F1703305";
char output2[]="CODE 517021910882";
char command[200];
char output[200];
uint8_t i=0;
...
void UART0_Handler(void)
{
	...
	while(UARTCharsAvail(UART0_BASE))
	{
		...
		*(command+i)=UARTCharGetNonBlocking(UART0_BASE);
		i++;
	}
	if(!strncmp(command,command1,8))
	{
		strcpy(output,output1);
		//output[strlen(output)]='\0';
		UARTStringPutNonBlocking(output);
	}
	else if(!strncmp(command, command2,14))
	{
		strncpy(output,output2,14);
		//output[strlen(output)]='\0';
		UARTStringPutNonBlocking(output);
	}
	//memset(output,0,200);
	//memset(command,0,200);
	//else 	UARTStringPutNonBlocking("NOT CORRECT COMMAND!");
...
char zhiling1[]="AT+CLASS";
char zhiling2[]="AT+STUDENTCODE";
char shuchu1[]="CLASS F1703305";
char shuchu2[]="CODE 517021910882";
char mingling[200];
char shuchu[200];
uint16_t i=0;
uint16_t flag=0;
...
int main(void)
{
  ...
  IntMasterEnable();		
	...
	while (1)
	{
		if (systick_10ms_status)
		{
			systick_10ms_status		= 0; //reset 10ms timer 
			if (++gpio_flash_cnt	>= GPIO_FLASHTIME/10)
			{
				gpio_flash_cnt			= 0;
...
void UART0_Handler(void)
{
  ...
  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
		...
		*(mingling+i) = UARTCharGetNonBlocking(UART0_BASE);
		i++;
	...
	//if(!UARTCharsAvail(UART0_BASE))
		//	i = 0;
	if(!strncmp(mingling,zhiling1,8))
		strcpy(shuchu,shuchu1);
	if(flag==0)
	{
		if(!strncmp(mingling,zhiling2,8))
			flag++;
	}
	else if(flag==1){
				if(!strcmp(mingling,"NTCODE"))
					flag++;
		}
