		...
		if(state!=3) first=1;
			if(systick_100ms_counter_status==1)
			{
				...
				if(++cnt_500 >= 10)  //1s
				{
					 ...
					 if(count==0) result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~(0x81<<count));
						else 
						result = I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~(3<<(count-1)));	
						//Delay(60000);
				/*
